...
if __name__ == "__main__":
    ...
    if mode == "predict":
        ...
        while True:
            ...
            #批量预测
            predict_path = input('Input image filepath:')
            save_path = input('Input image savepath:')
            temp_pred = os.listdir(predict_path)
            for pred in temp_pred:
                if pred.endswith(('.bmp', '.dib', '.png', '.jpg', '.jpeg', '.pbm', '.pgm', '.ppm', '.tif', '.tiff')):
                    try:
                        image = Image.open(os.path.join(predict_path,pred))
                    except:
                        print('Open Error! Try again!')
                        continue
                    else:
                        r_image = unet.detect_image(image, count=count, name_classes=name_classes)
                        r_image.save(os.path.join(save_path,pred))
